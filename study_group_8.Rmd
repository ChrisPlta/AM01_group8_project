...
Thanksgiving generates the highest weekly sales which is 22362.

```{r Sales Difference}
holiday_sales %>% 
  filter( holiday %in% c("Thanksgiving", "Labor day")) %>% 
   summarise(Name = "Values sales more than labor day", diff_sales = abs(diff(mean_weekly_sales)) )
```

The difference in average week sales between Thanksgiving and Labor day is 6368.
...
The difference of sales on holiday weeks and non-holiday weeks is not significant when filtering out Thanksgiving from the data since the P value from the T-test is greater than 5%. 
